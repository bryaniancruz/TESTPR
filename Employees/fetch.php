...
if(isset($_POST['load']) == "load"){
    ...
    $query = "SELECT * FROM userlists where isActive = 1" ;
    ...
    while($row = mysqli_fetch_array($result))
    {
        ...
        if($row['isLocked'] == 0)
        {
            ...
            <button type="button" name="delete" class="btn btn-warning btn-xs lock" id="'.$row["ID"].'">  <i class="fa fa-unlock"></i> </button>
...
if(isset($_POST['restore']) == "restore"){
    ...
    while($row = mysqli_fetch_array($result))
    {
        ...
        {
            $sub_array[] = '<td><center>
            <button type="button" name="delete" class="btn btn-warning btn-xs restore" id="'.$row["ID"].'">  <i class="fa fa-recycle"></i> </button>
